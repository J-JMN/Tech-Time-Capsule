...
def seed_db():
    ...
    cat2 = Category(name="Operating Systems", description="Software that manages computer hardware and software resources.", user_id=archivist.id)
    db.session.add_all([event1, event2, cat1, cat2])
    db.session.commit()

    assoc1 = EventCategory(event_id=event1.id, category_id=cat1.id, relationship_description="Revolutionized the mobile industry")
    assoc2 = EventCategory(event_id=event2.id, category_id=cat2.id, relationship_description="Dominated the desktop market in the 90s")
